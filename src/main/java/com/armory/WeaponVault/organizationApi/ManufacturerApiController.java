...
public class ManufacturerApiController implements ManufacturerApi {
    ...
    private final ManufacturerApiDelegate delegate;

    public ManufacturerApiController(@Autowired(required = false) ManufacturerApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new ManufacturerApiDelegate() {});
    }

    @Override
    public ManufacturerApiDelegate getDelegate() {
        return delegate;
    }
