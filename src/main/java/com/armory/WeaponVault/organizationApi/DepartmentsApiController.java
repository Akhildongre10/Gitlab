...
public class DepartmentsApiController implements DepartmentsApi {
    ...
    private final DepartmentsApiDelegate delegate;

    public DepartmentsApiController(@Autowired(required = false) DepartmentsApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new DepartmentsApiDelegate() {});
    }

    @Override
    public DepartmentsApiDelegate getDelegate() {
        return delegate;
    }
