...
public class InventoryApiController implements InventoryApi {
    ...
    public InventoryApiController(@Autowired(required = false) InventoryApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new InventoryApiDelegate() {});
    }

    @Override
    public InventoryApiDelegate getDelegate() {
        return delegate;
    }
